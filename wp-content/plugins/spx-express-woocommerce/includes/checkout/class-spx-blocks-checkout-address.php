...
final class SPX_Blocks_Checkout_Address {
	...
	public static function register_integration( $registry ): void { $registry->register( new SPX_Blocks_Checkout_Integration() ); }
	...
	public static function extend_store_api(): void {
		...
		if ( function_exists( 'woocommerce_store_api_register_update_callback' ) ) {
			woocommerce_store_api_register_update_callback( array( 'namespace'=>self::NAMESPACE, 'callback'=>array(__CLASS__,'update_cart_selection') ) );
		}
	}

	public static function update_cart_selection( $data ): void {
		$data=is_array($data)?$data:array();$clean=array();
		foreach(array('province_id','district_id','ward_id','dataset_version') as $key){$clean[$key]=isset($data[$key])?sanitize_text_field((string)$data[$key]):'';}
		$is_cod=function_exists('WC')&&WC()&&WC()->session&&'cod'===(string)WC()->session->get('chosen_payment_method','');
		$result=SPX_Classic_Checkout_Address::validate_payload($clean,$is_cod,new SPX_Checkout_Address_Service());
		if(empty($result['valid'])){throw new RouteException('spx_checkout_address_'.$result['error'],SPX_Classic_Checkout_Address::message_for_error($result['error']),400);}
		...
		foreach ( array( 'province_id', 'district_id', 'ward_id', 'dataset_version' ) as $field ) {
			$schema[ $field ] = array(
				...
				'arg_options' => array(
					...
					'validate_callback' => function ( $value ) { return is_string( $value ) && strlen( $value ) <= 32; },
				),
			);
		}
		return $schema;
	...
	public static function update_order_from_request( WC_Order $order, WP_REST_Request $request ): void {
		...
		$payload = isset( $extensions[ self::NAMESPACE ] ) && is_array( $extensions[ self::NAMESPACE ] ) ? $extensions[ self::NAMESPACE ] : array();
		$clean = array();
		foreach ( array( 'province_id', 'district_id', 'ward_id', 'dataset_version' ) as $key ) { $clean[ $key ] = isset( $payload[ $key ] ) ? sanitize_text_field( (string) $payload[ $key ] ) : ''; }
		$result = SPX_Classic_Checkout_Address::validate_payload( $clean, 'cod' === $order->get_payment_method(), new SPX_Checkout_Address_Service() );
		if ( empty( $result['valid'] ) ) {
			throw new RouteException( 'spx_checkout_address_' . $result['error'], SPX_Classic_Checkout_Address::message_for_error( $result['error'] ), 400 );
		}
		SPX_Checkout_Address_Snapshot::write_to_order( $order, $result['snapshot'], SPX_Checkout_Address_Snapshot::SOURCE_BLOCKS_CHECKOUT );
		SPX_Classic_Checkout_Address::remember_snapshot( $result['snapshot'] );
	}
	...
	public static function order_requires_selection( WC_Order $order ): bool {
		...
		foreach ( $order->get_items( 'line_item' ) as $item ) {
			...
			if ( $product && $product->needs_shipping() ) { $needs_shipping = true; break; }
		}
		$methods = array();
		foreach ( $order->get_items( 'shipping' ) as $item ) { $methods[] = $item->get_method_id(); }
