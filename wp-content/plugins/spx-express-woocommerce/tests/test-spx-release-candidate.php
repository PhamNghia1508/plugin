...
spx_rc_check( 1 === preg_match( '/^[ \t]*\*[ \t]+Version:[ \t]+0\.9\.0-rc\.10[ \t]*$/m', $main ), 'plugin header uses 0.9.0-rc.10' );
spx_rc_check( false !== strpos( $main, "define( 'SPX_WC_VERSION', '0.9.0-rc.10' );" ), 'runtime version constant uses 0.9.0-rc.10' );
spx_rc_check( false !== strpos( $bundle, "version: '0.9.0-rc.10'" ), 'Checkout Blocks metadata uses the RC version' );
spx_rc_check( false !== strpos( $changelog, '0.9.0-rc.10' ), 'changelog contains the RC version' );
